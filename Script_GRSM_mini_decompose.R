...
SNIRT_D_GRSM = function(y, J, nchain=2, Update=F, mod=NULL, epoch=300, nbatch=min(c(nrow(y),30)), iters=(epoch*N/nbatch)+1, burnin=round((iters-1)/3), thin=round((iters-burnin-1)/1000)){
  ...
  ptau = function(y,a,tp,tf,tau,St){
    ...
    m_a = array(matrix(a, N, N, byrow=T), dim=c(N,N,J-1))
    m_t = m_a * array(m_tp, dim=c(N,N,J-1))+array(m_tf, dim=c(N,N,J-1))
  ...
  pthetaP = function(y,a,tp,tf,tau,tfn,STT){
    ...
    tmp = (tp^2 - 2 * tp * tfn * STT[1,2])/(1-STT[1,2]^2)
  ...
  pthetaF = function(y,a,tp,tf,tau,tpn,STT){
    ...
    tmp = (tf^2 -2 * tpn * tf * STT[1,2])/(1-STT[1,2]^2)
  ...
  if(Update==F){
    ...
    tau = array(seq(-1.5,1.5,length.out=J-1),dim = c(J-1, iters, nchain))
    Sa = 0.5
    St = 0.5
    A = array(rep(0,1),dim=c(1,iters,nchain))
  }else{
    ### Chain
    a = array(mod$Estimate$a, dim = c(N, iters, nchain))
    tp = array(mod$Estimate$ThetaP, dim = c(N, iters, nchain))
    tf = array(mod$Estimate$ThetaF, dim = c(N, iters, nchain))
    tau = array(mod$Estimate$Tau, dim = c(J-1, iters, nchain))
    Sa = 0.5
    St = 0.5
    A = -mod$Estimate$Rho
  }
    ...
    for(e in 1:epoch){
      for(b in 1:BatchLen){
        ...
        ### tau
        {
          ...
          R = exp(ptau(y[batch,batch],a[batch,iter,c], tp[batch,iter,c],tf[batch,iter,c],proposal,St)
                  -ptau(y[batch,batch],a[batch,iter,c], tp[batch,iter,c],tf[batch,iter,c],tau[,iter,c],St) )
          if(is.na(R)==T){R=0}
          accept = rbinom(1,1,min(1,R))
          tau[,iter+1,c] = ifelse(rep(accept,K-1),proposal,tau[,iter,c])
        }
        Jtau = apply(tau[,1:(iter+1),c],1,var)[1:(J-2)] + rep(0.00001,J-2)
        ...
        for(n in 1:N){
          n_theta = unique(c(n, batch))[-1]
          {
            ...
            R = exp(log(proposal)+pa(y[n_theta,n],proposal,tp[n_theta,iter,c],tf[n,iter,c],tau[,iter+1,c],Sa)
                -log(a[n,iter,c])-pa(y[n_theta,n],a[n,iter,c],tp[n_theta,iter,c],tf[n,iter,c],tau[,iter+1,c],Sa) )
            R[is.na(R)] = 0
            accept = rbinom(1,1,min(1,R))
            ...
            R = exp(pthetaF(y[n_theta,n],a[n,iter+1,c],tp[n_theta,iter,c],proposal,tau[,iter+1,c],tp[n,iter,c],STT)-
                    pthetaF(y[n_theta,n],a[n,iter+1,c],tp[n_theta,iter,c],tf[n,iter,c],tau[,iter+1,c],tp[n,iter,c],STT))
            if(is.na(R)==T){R=0}
            accept = rbinom(1,1,min(1,R))
        ...
        for(n in 1:N){
          n_theta = unique(c(n, batch))[-1]
          {
            ...
            R = exp(pthetaP(y[n,n_theta],a[n_theta,iter+1,c],proposal,tf[n_theta,iter+1,c],tau[,iter+1,c],tf[n,iter+1,c],STT)-
                    pthetaP(y[n,n_theta],a[n_theta,iter+1,c],tp[n,iter,c],tf[n_theta,iter+1,c],tau[,iter+1,c],tf[n,iter+1,c],STT))
            if(is.na(R)==T){R=0}
            accept = rbinom(1,1,min(1,R))
