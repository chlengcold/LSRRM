...
SNIRT_D_PCM = function(y, J, nchain=2, epoch=300, nbatch=min(c(nrow(y),30)), iters=(epoch*N/nbatch)+1, burnin=round((iters-1)/3), thin=round((iters-burnin-1)/1000)){
  
  if(nrow(y)%%nbatch!=0){
    cat('nbatch is unavailable.\n')
    break()
  }
      ...
      for(b in 1:BatchLen){
        ...
        for(n in 1:N){
          n_theta = unique(c(n, batch))[-1]
          {
            ...
            R = exp(ptau(y[n_theta,n],tp[n_theta,iter,c],tf[n,iter,c],proposal,St[n])
                   -ptau(y[n_theta,n],tp[n_theta,iter,c],tf[n,iter,c],tau[n,,iter,c],St[n]) )
            if(is.na(R)==T){R=0}
            accept = rbinom(1,1,min(1,R))
            ...
            R = exp(pthetaF(y[n_theta,n],tp[n_theta,iter,c],proposal,tau[n,,iter+1,c],tp[n,iter,c],STT)-
                    pthetaF(y[n_theta,n],tp[n_theta,iter,c],tf[n,iter,c],tau[n,,iter+1,c],tp[n,iter,c],STT))
            if(is.na(R)==T){R=0}
            accept = rbinom(1,1,min(1,R))
        ...
        for(n in 1:N){
          n_theta = unique(c(n, batch))[-1]
          {
            ...
            R = exp(pthetaP(y[n,n_theta],proposal,tf[n_theta,iter+1,c],tau[n_theta,,iter+1,c],tf[n,iter+1,c],STT)-
                    pthetaP(y[n,n_theta],tp[n,iter,c],tf[n_theta,iter+1,c],tau[n_theta,,iter+1,c],tf[n,iter+1,c],STT))
            if(is.na(R)==T){R=0}
            accept = rbinom(1,1,min(1,R))
